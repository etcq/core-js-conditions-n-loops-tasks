...
function getSpiralMatrix(size) {
  ...
  while (row <= rowEnd && col <= colEnd) {
    ...
    for (let j = row; j <= rowEnd; j += 1) {
      arr[j][colEnd] = counter;
      counter += 1;
    }
    colEnd -= 1;
    for (let x = colEnd; x >= col; x -= 1) {
      arr[rowEnd][x] = counter;
      counter += 1;
    }
    rowEnd -= 1;
    for (let y = rowEnd; y >= row; y -= 1) {
      arr[y][col] = counter;
      counter += 1;
    }
    col += 1;
...
function sortByAsc(arr) {
  const partition = (array, start, end) => {
    ...
    [rarr[i], rarr[end]] = [rarr[end], rarr[i]];
    return i;
  };
  const quickSort = (sortArr, start, end) => {
    if (start < end) {
...
function getNearestBigger(number) {
  ...
  for (let j = arrNum.length - 1; j >= i; j -= 1) {
    if (arrNum[i - 1] < arrNum[j]) {
